...
section = st.sidebar.radio("Select a section:", ["Introduction", "EDA", "Prediction", "Recommendation"])

# ============= INTRODUCTION SECTION =============
if section == "Introduction":
    st.title("Welcome to the Credit Default Prediction Dashboard")
    st.markdown("""
    <div style='font-size:20px; line-height:1.8; text-align: justify;'>
    This dashboard helps predict whether an e-commerce customer will default on a loan. 
    By studying their loan and transaction history, we aim to improve credit decisions. 
    With better credit assessments, we can help both the company and its customers.
    </div>
    """, unsafe_allow_html=True)

    # Smaller, eye-catching image for introduction
    st.image('Intro.png', width=400)  # Fintech image
...
# ============= RECOMMENDATION SECTION =============
    ...
    st.title("Our Recommendation for Xente")
    
    # Adding a colorful divider for better visual organization
    st.markdown("<hr style='border: 2px solid #f3c623; margin-top: 20px; margin-bottom: 20px;'>", unsafe_allow_html=True)
    
    # Displaying key recommendation message in a visually enhanced box
    st.markdown("""
    <div style='background-color: #f3f4f6; padding: 20px; border-radius: 10px; font-size:20px; line-height:1.8; text-align: justify;'>
    We recommend that Xente keeps monitoring credit risks by tracking customer transactions and loan history. 
    This data can help lower default rates and improve decision-making, ensuring financial growth for both Xente 
    and its clients. Additionally, implementing advanced analytics will provide further insights into customer behavior.
    </div>
    """, unsafe_allow_html=True)
    
    # Highlighting a key metric with a dynamic stat
    st.metric("Default Rate Reduction", "-15%", "Improvement in Q2 2024")
    
    # Smaller, eye-catching image for recommendation
    st.image('Introo.png', width=400)  # Business growth image
    
    
    # Another colorful divider to wrap up the section
    st.markdown("<hr style='border: 2px solid #f3c623; margin-top: 20px; margin-bottom: 20px;'>", unsafe_allow_html=True)
